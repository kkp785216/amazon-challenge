...
export const initialState = {
    cart: [],
    loginUser: (async () => {
        return window.localStorage.getItem('user_amazon_challenge') ?
                ...
                try {
                    ...
                    if (q1Data.length >= 1 || q2Data.length >= 1) {
                        ...
                        if (((q1Data.length >= 1 && q1Data[0]) || (q2Data.length >= 1 && q2Data[0])).password === action.password) {
                            window.localStorage.setItem('user_amazon_challenge', JSON.stringify(logedInUser));
                            return { logedIn: true, alert: 'success', message: 'Login Successfully', user: logedInUser }
                        }
                        else {
                            window.localStorage.setItem('user_amazon_challenge', null);
